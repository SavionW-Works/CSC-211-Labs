...
int main()
{
	//Problem 1: UNCOMMENT BEFORE SUBMISSION
	int number; 
	int m = 0; 
	int y = 0;
	cout << "Enter a number from 1 to 15 for your pyramid:";
	cin >> number;
	Pyramid(number);
	//END OF (Problem 1)  
	cout << "\n Enter a month and year to display the days in the month: "; 
	cin >> m >> y; 
	cout << " \n (NOTE: Starting day of month defaults to Sunday and may not be accurate) \n";
	DaysOfMonth(m, y);
}


int DaysOfMonth(int month_num, int year) { 
	int month = month_num; 
	int day_max;  
	int week_day = 0; 
	int day = 1; 
	...
	cout << "\n\n" << month_interp(month) << " has " << day_max << " days";
